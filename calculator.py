...
class Calculator:
    def __init__(self, master):
        self.master = master
        self.display = tk.Entry(master, width=15, font=("Arial", 23), bd=10, insertwidth=1, bg="#27D3F5", justify="right")
    ...
    def click(self, key):
        ...
        print(f"\n[+] You had pressed the button {key}")
        print(f"[+] The first combination is: {self.current}")
        print(f"[+] Status op_verification: {self.op_verification}")
        print(f"[+] Operation type that you wish to do: {self.op}")
    ...
    def build_button(self, button, row, col):
        if button == "C":
            ...
            b = tk.Button(self.master, text=button, width=4, command=lambda: self.click(button))
